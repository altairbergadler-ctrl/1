      ...
      <main>
        <section class="page-header">
          ...
          <div class="action-row">
            ...
            ${playlist.summary.missing > 0 && qobuzStatus?.enabled && qobuzStatus?.configured ? `<button class="secondary" type="button" data-action="qobuz-fetch" data-playlist-id="${playlist.id}" ${qobuzJobOpen || qobuzEligible === 0 ? "disabled" : ""}>${qobuzJob?.paused_at ? "Qobuz: загрузка на паузе" : qobuzDownloadActive ? "Qobuz: загрузка выполняется…" : qobuzEligible === 0 ? "Qobuz: новых треков нет" : `⬇ Проверить новые в Qobuz (${qobuzEligible})`}</button>` : ""}
