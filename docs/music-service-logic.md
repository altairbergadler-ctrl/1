...
### 6.5 Автоматический acquisition workflow

Любой успешный импорт автоматически создаёт `acquisition_workflow`.
Предварительный matching отбирает `MISSING`, а режим «Обновлять качество»
добавляет `READY`. Каждая позиция последовательно проверяется в Qobuz и
Яндекс Музыке; скачивается только лучший доступный вариант. Для READY новая
копия сохраняется лишь если её quality tuple лучше текущей.

Одна глобальная provider-пачка содержит 25 позиций, не 25 файлов. Dispatcher
чередует пользователей round-robin и не допускает два активных задания одного
плейлиста. После пачки выполняются import → scan → Drive upload → проверка →
локальная эвакуация. Межпакетный таймер идёт параллельно drain-фазе, но
следующая пачка не стартует до её успешного завершения и проверки свободного
места.

Курсор, file IDs и provider attempts сохраняются в PostgreSQL. Поэтому
продолжение после паузы или процесса recovery не повторяет уже сохранённые
позиции. После последней пачки выполняется финальный matching.

PWA получает безопасные агрегаты: позиции, скачанные файлы, Drive uploads,
локальные эвакуации, этап и свободный диск. Browser Push на стандартном VAPID
отправляется только один раз после полного workflow; промежуточные этапы не
уведомляют. Subscription secrets шифруются в server vault. Подробности:
