...
Текущий head — `0012_web_push_subscriptions`. Сервис `migrate` выполняет
последовательные Alembic-миграции и безопасно
...
- `POST /api/playlists/import` — фоновый импорт всех доступных плейлистов
  источника по `source_id` с последующей автоматической загрузкой;
...
в автоматическую очередь загрузки. Один запрос ограничен 2 МБ и 5000 треками.
Он не читает Spotify-ссылку без OAuth. Форматы, API и границы
безопасности описаны в [`docs/playlist-import.md`](docs/playlist-import.md).


## Автоматическая загрузка и очередь

После импорта из любого источника Qobuz и Яндекс Музыка последовательно
проверяют каждую позицию, сервис выбирает лучшее доступное качество, скачивает
только победивший вариант и после всех пачек запускает итоговый matching.
Обычный режим обрабатывает `MISSING`; режим «Обновлять качество» также
проверяет `READY` и скачивает файл только при реальном улучшении.

Пачка — это 25 позиций плейлиста, поэтому скачанных файлов может быть меньше.
Между пачками выполняются import, scan, проверенная загрузка в Google Drive и
удаление локальной копии. Очередь допускает одну глобальную provider-пачку и
справедливо чередует пользователей. При свободном месте менее 5 GiB следующая
пачка не начинается.

- `POST /api/acquisition/playlists/{id}` — поставить или вернуть активное
  задание;
- `GET /api/acquisition/playlists/{id}` — прогресс и текущий этап;
- `POST /api/acquisition/jobs/{id}/pause` — пауза после активной пачки;
- `POST /api/acquisition/jobs/{id}/resume` — продолжение с сохранённого
  курсора.

PWA показывает позиции, скачанные и загруженные файлы, локальную очистку,
этап и свободный диск. Browser Push отправляется только один раз после всех
пачек и финального matching. Подробный контракт и production runbook:
...
кэш не попадают. Web Push использует стандартный VAPID и не требует отдельного
Google/Firebase приложения; подписка создаётся только после пользовательского
разрешения, а endpoint и browser keys хранятся зашифрованно. Session и CSRF
tokens не записываются в Web Storage. Nginx
...
- `QOBUZ_MIN_FREE_BYTES` (по умолчанию 5 GiB) — автоматическая пауза до новой
  пачки, если на разделе staging осталось меньше свободного места;
...
- `POST /api/qobuz/downloads/{job_id}/pause` — запрос безопасной паузы после
  сохранения текущей пачки;
- `POST /api/qobuz/downloads/{job_id}/resume` — продолжение той же job по
  сохранённому provider ledger без повторной загрузки завершённых треков.
...
PWA показывает скачивание и дренирование текущего пакета, ручные действия
«Пауза после пачки» / «Продолжить» и статусы треков: `queued`, `searching`,
`downloading`, `stored`, `not_found`, `ambiguous`, `conflict` или `failed`.
При кратком обрыве API интерфейс переподключается и продолжает следить за той
же job.

После каждой пачки worker переносит проверенные файлы из staging в библиотеку,
сканирует каталог, реплицирует файлы в настроенное durable storage и удаляет
локальные байты только после подтверждённой загрузки. Лишь затем начинается
межпакетная пауза и следующая пачка. Ручная пауза и disk guard срабатывают на
этой безопасной границе; уже завершённые позиции сохраняются в
`provider_attempts`, поэтому resume обрабатывает только оставшиеся.
