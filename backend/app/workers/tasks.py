...
from app.services.acquisition import AcquisitionPause, process_acquisition_batch
from app.services.acquisition_queue import (
    kick_acquisition_dispatcher,
    queue_source_playlists,
)
    ...
    try:
        ...
        updated_id = db.scalar(
            ...
            .where(
                Job.id == job_id,
                Job.status.in_([JobStatus.pending, JobStatus.running]),
                or_(Job.lock_owner.is_(N×½5ÞÚ$z{-®éÜj×lth_check_account(db, account)
            db.commit()
            items.append(
                {
